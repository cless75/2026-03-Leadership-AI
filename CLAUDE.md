...
3. Если ПЗ нет — по договорённости создать ПЗ по шаблону [[My-Notes/TEMPLATE-PZ]] (секции: Контекст, Ключевые решения, Ход работы, Открытые вопросы, Артефакты).

**Где хранить ПЗ:**

| Тип ПЗ | Путь | Видимость |
|--------|------|-----------|
| Дефолтный (приватный для участника) | `My-Notes/PZ/<ProjectId>-п-<Name>.md` | в `.gitignore` (`My-Notes/**/*.md`) — не коммитится |
| Тематический (хочу видеть в daily timeline) | `2026/<тема>/ПЗ-<Name>.md` | папка `2026/` целиком в `.gitignore` |

**Не класть** ПЗ в корень vault `504-demo/` — там файлы попадают под git и могут случайно закоммититься. По умолчанию все ПЗ — в `My-Notes/PZ/` (см. шаблон [[My-Notes/TEMPLATE-PZ]] и индекс [[My-Notes/README]]).
...
| dashboard прогресса, render notes, html-обзор недели | `/render-notes` | render-notes-html | HTML на `Desktop/<участник>/dashboard-YYYY-MM-DD.html` (timeline, EQ, статус, эксперименты) |
